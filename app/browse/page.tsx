...
import { Search, Clapperboard, SlidersHorizontal } from 'lucide-react'
import type { ListingWithImages, ListingCategory, ListingCondition } from '@/lib/types/database'
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export default async function BrowsePage({ searchParams }: BrowsePageProps) {
  ...
  if (params.categories) {
    const cats = params.categories.split(',').filter(Boolean) as ListingCategory[]
    if (cats.length > 0) {
  ...
  if (params.conditions) {
    const conds = params.conditions.split(',').filter(Boolean) as ListingCondition[]
    if (conds.length > 0) {
      ...
      <header className="sticky top-0 z-50 glass border-b border-border/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 gap-4">
            ...
            <form action="/browse" className="flex-1 max-w-lg hidden sm:block">
      ...
      <div className="border-b border-border/50 bg-background/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            <Link href="/browse">
              <Button
                variant={!params.categories ? 'default' : 'outline'}
                size="sm"
            ...
            {categories.slice(0, -1).map(category => {
              ...
              const isActive = params.categories?.split(',').includes(category.slug)
              return (
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-2xl font-bold">
                  {params.q ? `Results for "${params.q}"` : 'Browse All'}
                </h1>
